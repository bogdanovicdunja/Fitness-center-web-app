    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for (i = 0; i < res.length; i++) {                              // prolazim kroz listu svih treninga
                ...
                row += "<td> <form id='preusmerenje' method='post'> <input type='hidden' value='" + res[i].id +  " '> <input type='submit' value='Izmena'></form> </td>";
                //SKRIVENI ID TERMINA koji korisnik bira idTermina
...
//******* TRENER IZABRAO KOJI CE TERMIN DA IZMENI ********
$(document).on("submit", "#preusmerenje", function (event) {
    event.preventDefault();

    // preuzimamo vrednost tog termina sto je skriveni id od gore nakon klika na submit dugme Izmena
    var id = $(this).find('input:hidden').val();
    window.localStorage.setItem('idTermina', id);    //SACUVAMO ga kao idTermina
    window.location.href = 'termin_izmeni.html';    //preusmerenje na tu str. nakon klika na dugme Prijava
